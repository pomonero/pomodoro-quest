    ...
    <div className="card p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <span className="font-medium text-sm" style={{ color: 'var(--text)' }}>{t.didYouKnow}</span>
        </div>
        <button onClick={getRandomFact} className="p-2 rounded-lg hover:bg-[var(--surface-hover)]" style={{ color: 'var(--text-muted)' }}>🔄</button>
      </div>
      <div className="flex items-start gap-3 p-3 rounded-xl" style={{ background: 'var(--surface)' }}>
        <span className="text-2xl">{fact.icon}</span>
        <p className="text-sm leading-relaxed" style={{ color: 'var(--text)' }}>{fact.text}</p>
      </div>
    </div>
  );
}

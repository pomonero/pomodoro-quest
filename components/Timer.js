...
export default function Timer() {
  ...
  const sessionConfig = {
    work: { icon: '🎯', color: 'var(--primary)', label: t.focus },
    break: { icon: '☕', color: '#22c55e', label: t.shortBreak },
    longBreak: { icon: '🌴', color: '#f97316', label: t.longBreak }
  };
  ...
  return (
    <div className="card p-6">
      ...
      <div className="flex justify-center gap-2 mb-6">
        {Object.entries(sessionConfig).map(([type, cfg]) => (
          <button
            key={type}
            onClick={() => changeSession(type)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
              sessionType === type ? 'text-white shadow-lg' : ''
            }`}
            style={{ 
              background: sessionType === type ? cfg.color : 'var(--surface)',
      ...
      <div className="relative flex justify-center items-center mb-6">
        ...
        <div className="absolute flex flex-col items-center">
          ...
          <span className="timer-display text-5xl md:text-6xl font-bold" style={{ color: 'var(--text)' }}>
            {formatTime(timeLeft)}
          </span>
          <span className="text-sm mt-2" style={{ color: 'var(--text-muted)' }}>{config.label}</span>
        </div>
      </div>
      ...
      <div className="flex justify-center gap-2 mb-6">
        {[1, 2, 3, 4].map((i) => {
          const filled = i <= (completedSessions % 4 || (completedSessions > 0 && completedSessions % 4 === 0 ? 4 : 0));
          return (
            <div
              key={i}
              className={`w-8 h-8 rounded-lg flex items-center justify-center text-lg transition-all ${
                filled ? 'shadow-lg animate-sparkle' : ''
              }`}
              style={{ background: filled ? 'var(--primary)' : 'var(--surface)' }}
            >
              {filled ? '⭐' : '☆'}
            </div>
          );
        ...
        <div className="mt-6 p-4 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--primary)' }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <p className="font-medium" style={{ color: 'var(--text)' }}>{t.breakTime}</p>
            </div>
            <button

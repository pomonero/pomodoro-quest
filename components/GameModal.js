...
export default function GameModal() {
  ...
  const closeModal = () => { setShowGame(false); setSelectedGame(null); setGameState('select'); setScore(0); if (gameLoopRef.current) cancelAnimationFrame(gameLoopRef.current); };

  const startGame = (game) => { setSelectedGame(game); setGameState('playing'); setScore(0); };
  ...
  useEffect(() => {
    ...
    if (selectedGame.id === 'runner') {
      ...
      const loop = () => {
        ...
        ctx.fillStyle = '#fff'; ctx.font = '20px Arial'; ctx.fillText('🚀', player.x + 5, player.y + 22);
        ...
        obstacles = obstacles.filter(obs => {
          ...
          ctx.fillStyle = '#fff'; ctx.font = '16px Arial'; ctx.fillText('☄️', obs.x + 4, obs.y + 20);
      ...
      const loop = () => {
        ...
        if (cameraY < highest + 200) {
          for (let i = 0; i < 3; i++) platforms.push({ x: Math.random() * 320, y: highest - 50 - i * 50, width: 70 });
        }
      ...
      const loop = () => {
        ...
        enemies = enemies.filter(e => {
          ...
          if (player.x < e.x + e.size - 4 && player.x + player.size - 4 > e.x && player.y < e.y + e.size - 4 && player.y + player.size - 4 > e.y) {
            endGame(gameScore); return false;
          }
        ...
        {gameState === 'select' && (
          <>
            ...
            <div className="grid grid-cols-1 gap-3">
              {games.map((game) => (
                <button key={game.id} onClick={() => canPlayGame && startGame(game)} disabled={!canPlayGame} className={`flex items-center gap-4 p-4 rounded-xl text-left transition-all ${canPlayGame ? 'hover:border-[var(--primary)] cursor-pointer' : 'opacity-50 cursor-not-allowed'}`} style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                  <span className="text-3xl">{game.icon}</span>
                  <div className="flex-1"><h3 className="font-bold" style={{ color: 'var(--text)' }}>{t[game.name]}</h3><p className="text-sm" style={{ color: 'var(--text-muted)' }}>{t[game.desc]}</p><p className="text-xs mt-1" style={{ color: 'var(--primary)' }}>{t[game.controls]}</p></div>
                </button>
              ))}
            </div>
          </>
